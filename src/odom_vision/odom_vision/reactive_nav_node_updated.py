...
class ReactiveNavNode(Node):
    ...
    def __init__(self):
        ...
        self.slow_distance = 0.01* max(3.0, 2.0 * self.cruise_speed)
        self.stop_distance = 0.5 * max(1.5, 0.75 * self.cruise_speed)
        ...
        self.declare_parameter("velocity_weight", 0.7)
        ...
        self.declare_parameter("dwa_rate", 20.0)
        self.declare_parameter("occupied_threshold", 50)  # OctoMap cell value to treat as solid
        self.declare_parameter("inflation_radius", 0.5)  # meters — lethal + soft inflation extent
        ...
        self.declare_parameter("emergency_distance_base", 1.3)
        ...
        self._astar_lock = threading.Lock()  # guards global_path writes
        self._astar_running = False  # True while a replan thread is live
    ...
    def _publish_inflated_grid(self):
        ...
        data = (self.cost_grid * 100.0).astype(np.int8).flatten()
    ...
    def _is_trajectory_safe(self, trajectory):
        ...
        # Anchor the skip-zone on the trajectory's actual origin (which is the
        # latency-compensated position from _simulate_trajectory), NOT on
        # self.drone_x/y. Otherwise the ~150 ms forward-projection offset can
        # leave trajectory[0] outside the skip radius, and we'd immediately
        # check cells under the drone's footprint — triggering phantom-lethal
        # rejections on every trajectory.
        if len(trajectory) > 0:
            skip_x, skip_y = trajectory[0][0], trajectory[0][1]
        else:
            skip_x, skip_y = self.drone_x, self.drone_y

        skip_radius = self.drone_radius
        skip_r2 = skip_radius * skip_radius

        prev_x, prev_y = skip_x, skip_y
    ...
    def _score_trajectory(self, trajectory, v):
        ...
        # ---- Velocity-dependent weights with user-parameter scaling ----
        v_ratio = v / self.max_v if self.max_v > 0 else 0.0

        # Base weights encode physics: near obstacle → clearance dominates
        #                               open space  → progress dominates
        base_progress = 0.10 + 0.25 * v_ratio
        base_clearance = 0.75 - 0.35 * v_ratio
        base_alignment = 0.10
        base_velocity = 0.05

        # User parameters scale each term (1.0 = default behavior unchanged)
        w_progress = self.heading_weight * base_progress
        w_alignment = self.heading_weight * base_alignment
        w_clearance = self.clearance_weight * base_clearance
        w_velocity = self.velocity_weight * base_velocity

        # Normalize to sum = 1.0 so scores stay in [0, 1]
        total = w_progress + w_alignment + w_clearance + w_velocity
        if total > 1e-6:
            w_progress /= total
            w_alignment /= total
            w_clearance /= total
    ...
    def _get_local_goal(self) -> Optional[Tuple[float, float]]:
        ...
        with self._astar_lock:
            path = list(self.global_path)  # snapshot so DWA reads a stable copy
        if not path:
            return None
    ...
    def _trigger_replan(self, reason: str = ""):
        """
        Spawn A* in a background thread so _dwa_cycle is never blocked.
        If a replan is already running, skip — the in-flight thread will
        update global_path when it finishes.
        """
        if self.final_goal is None:
            return
        if self._astar_running:
            return  # previous replan still computing — don't pile up threads
        self._last_replan_time = time.time()  # debounce immediately
        self._astar_running = True
        # Snapshot pose/goal so the thread doesn't race with DWA state updates
        start_xy = (self.drone_x, self.drone_y)
        goal_xy = (self.final_goal[0], self.final_goal[1])
        t = threading.Thread(
            target=self._astar_worker,
            args=(start_xy, goal_xy, reason),
            daemon=True,
        )
        t.start()

    def _astar_worker(
        self, start_xy: Tuple[float, float], goal_xy: Tuple[float, float], reason: str
    ):
        """Background thread: run A*, swap path atomically, log timing."""
        t0 = time.time()
        try:
            path = self._plan_astar(start_xy, goal_xy)
            elapsed_ms = (time.time() - t0) * 1000.0
            if elapsed_ms > 100.0:
                self.get_logger().warn(
                    f"A* replan ({reason}): SLOW — {elapsed_ms:.0f} ms "
                    f"(may starve DWA on real hardware)"
                )
            with self._astar_lock:
            ...
            if path:
                self.get_logger().info(
                    ...
                    f"length={self._path_length(path):.1f}m  [{elapsed_ms:.0f}ms]"
                )
            else:
                self.get_logger().warn(
                    f"A* replan ({reason}): FAILED (no path) [{elapsed_ms:.0f}ms]"
                )
    ...
    def _dwa_cycle(self):
        ...
        # [FIX] Use ROS 2 clock instead of time.time() for simulation compatibility
        now_sec = self.get_clock().now().nanoseconds / 1e9
        need_replan = False
        if not self.global_path:
            need_replan = True
        elif now_sec - self._last_replan_time > self.replan_period:
            need_replan = True
        if need_replan:
        ...
        if best_traj is None:
            ...
            if self._stuck_start is None:
                self._stuck_start = now_sec  # [FIX] Use ROS time
            elif now_sec - self._stuck_start > self.stuck_replan_threshold:  # [FIX] Use ROS time
                self._trigger_replan(reason="stuck")
                self._stuck_start = now_sec  # debounce
        else:
            # [FIX] Execution-time brake cap: Remove safety_margin and allow reversing
            forward_dist = self._check_direction(self.drone_yaw, 8.0)
            v_cap = self.max_v

            if forward_dist is not None:
                # Subtract ONLY the physical drone_radius (plus a tiny 2cm buffer).
                # We drop self.safety_margin here because the DWA costmap already handles it.
                usable_dist = forward_dist - self.drone_radius - 0.02
            ...
            # Apply v_cap ONLY to forward motion. Let the drone reverse if needed!
            if best_v >= 0.0:
            ...
            # meaningful yaw, override into explicit turn-in-place.
            if v_cap < 0.05 and abs(best_w) < 0.05:
                ...
                # [FIX] >= 0 instead of > 0 so it doesn't spin away if already aligned perfectly
